...
void	set_hooks(t_vars *vars)
{
	...
	mlx_key_hook(vars->win, key_zoomup_hook, vars);
	mlx_key_hook(vars->win, key_zoomdown_hook, vars);
...
static int	key_zoomup_hook(int key, t_vars *vars)
{
	if (key == ESCAPE)
		mlx_loop_end(vars->mlx);
	return (EXIT_SUCCESS);
...
static int	key_zoomdown_hook(int key, t_vars *vars)
{
	if (key == ESCAPE)
		mlx_loop_end(vars->mlx);
	return (EXIT_SUCCESS);
